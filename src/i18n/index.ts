...
export function t(key: keyof Messages): string {
  if (!_messages) throw new Error('i18n not initialized. Call initI18n() first.')
...
if (import.meta.vitest) {
  ...
  describe('detectLocale', () => {
    ...
    afterEach(() => {
      ...
      for (const key of ['CLAUDE_CONFIG_LANG', 'LANG', 'LC_ALL', 'LC_MESSAGES']) {
        if (key in orig) {
  ...
  describe('initI18n + t()', () => {
    ...
    it('ja messages cover all en keys (no missing translations)', async () => {
      ...
      const jaKeys = Object.keys(ja.messages)
